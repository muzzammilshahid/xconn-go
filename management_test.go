...
func TestManagementStatsAPIs(t *testing.T) {
	...
	require.NoError(t, callResp.Err)
	expected := map[string]any{mgmtKeyInterval: int64(100), "running": true}
	require.Equal(t, expected, callResp.ArgDictOr(0, xconn.Dict{}).Raw())
	...
	callResp = session.Call(xconn.ManagementProcedureStatsStatusGet).Do()
	require.NoError(t, callResp.Err)
	expected = map[string]any{mgmtKeyInterval: int64(100), "running": false}
	require.Equal(t, expected, callResp.ArgDictOr(0, xconn.Dict{}).Raw())
