...
// StreamHandler is called on the server for each non-WAMP yamux stream opened by the client.
type StreamHandler func(session BaseSession, stream net.Conn)

// YamuxConnHandler is called once per yamux connection after WAMP authentication succeeds.
// ctx is cancelled when the yamux connection closes.
// conn allows the server to open streams back to the connected client.
type YamuxConnHandler func(ctx context.Context, session BaseSession, conn *YamuxClientConn)
